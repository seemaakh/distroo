...
export default function AdminAnnouncementsPage() {
  ...
  const { data: announcements = [], isLoading } = useQuery<Announcement[]>({
    ...
    queryFn: () => api.get("/announcements?all=1").then((r) => r.data),
  });

  const save = useMutation({
    mutationFn: (data: { message: string; active: boolean }) =>
      editing
        ? api.patch(`/announcements/${editing.id}`, data)
  ...
  const toggleActive = useMutation({
    mutationFn: ({ id, active }: { id: number; active: boolean }) =>
      api.patch(`/announcements/${id}`, { active }),
  ...
  const tickerText =
    activeMessages.length > 0
      ? activeMessages.map((a) => a.message).join(" · ")
      : "No active announcements";

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="font-grotesk font-bold text-2xl text-ink">
          Announcements
        </h1>
        <button
          onClick={openAdd}
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-16 bg-blue-pale rounded-2xl animate-pulse" />
          ))}
        </div>
          ...
          {announcements.map((a) => (
            <div
              ...
              className={`bg-white border rounded-2xl p-4 flex items-center gap-4 transition-colors ${
                a.active ? "border-blue" : "border-gray-200"
              }`}
            >
              <div className="flex-1 min-w-0">
                <p
                  className={`text-sm font-medium ${
                    a.active ? "text-ink" : "text-gray-400"
                  }`}
                >
                  {a.message}
                </p>
                <p className="text-xs text-gray-400 mt-1">
                  {new Date(a.createdAt).toLocaleDateString("en-NP")}
                </p>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button
                  onClick={() =>
                    toggleActive.mutate({ id: a.id, active: !a.active })
                  }
                  className={a.active ? "text-green" : "text-gray-400"}
                >
          ...
          <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl z-50 p-6 w-full max-w-md shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <button
                  onClick={() =>
                    save.mutate({ message, active: editing ? editing.active : true })
                  }
